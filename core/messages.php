<?php
    const MESSAGES = [

        /* 404 */
        'error_404' => 'Trang Không Tồn Tại',
        'error_404_message' => 'Trang bạn đang truy cập có thể đã bị xóa, bị thay đổi hoặc bạn không đủ quyền để truy cập.',

        /* form */
        'input_empty' => 'Vui lòng điền đầy đủ các thông tin',
        'username_used' => 'Username của bạn đã được sử dụng',
        'password_confirmpassword_notsame' => 'Mật khẩu và Xác nhận mật khẩu phải khớp với nhau',
        'emailtype_error' => 'Email của bạn không hợp lệ',
        'email_used' => 'Email đã được sử dụng',
        'nametype_error' => 'Tên chỉ được dùng chữ hoa/thường hoặc số và không có khoảng trắng (ví dụ: admin123)',
        'passwordtype_error' => 'Mật khẩu tối thiểu 6 ký tự, có chữ cái và số',
        'category_not_found' => 'Không tìm thấy category!',
        'login_failed' => 'Thông tin tài khoản hoặc mật khẩu không chính xác.',

        /* upload file */
        'upload_file_error' => 'Có lỗi xảy ra! Upload file thất bại',
        'file_type_error' => 'Định dạng file không đúng! (.zip/.rar)',
        'image_type_error' => 'Định dạng hình ảnh không đúng! (.jpg/.jpeg/.png)',
        'file_size_exceed' => 'Dung lượng file quá lớn!',
        'upload_file_success' => 'Upload file thành công!',

        /* product */
        'empty_product' => 'Không tìm thấy sản phẩm yêu cầu',

        /* payment */
        'payment_success' => 'Thanh toán thành công',
        'payment_success_message' => 'Hóa đơn của bạn đã được thanh toán thành công. Vui lòng đánh giá mức độ hài lòng của sản phẩm sau khi sử dụng. Chúc bạn có những trải nghiệm tuyệt vời trên hệ thống.',
        'payment_failed' => 'Thanh toán thất bại',
        'payment_failed_message' => 'Không thể thanh toán hóa đơn mua hàng do bạn không đủ tiền để chi trả. Vui lòng nạp thêm hoặc chọn phương thức thanh toán khác.',
    ];